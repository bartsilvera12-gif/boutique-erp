...
function UsuarioDetailContent() {
  ...
  // Borrado del usuario (soft delete vía DELETE endpoint).
  ...
  async function handleBorrar() {
    ...
    try {
      const res = await fetchWithSupabaseSession(`/api/empresas/usuarios/${id}`, { method: "DELETE" });
      ...
      setBorrarError(e instanceof Error ? e.message : "No se pudo borrar el usuario.");
    } finally {
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {!editing && (
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => { setBorrarOpen(true); setBorrarError(null); }}
              className="inline-flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg border border-red-200 hover:bg-red-50 transition-colors text-red-600"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
      ...
      <ConfirmModal
        ...
        message={usuario ? `¿Borrar a "${usuario.nombre ?? usuario.email}"?\n${usuario.email}` : ""}
        hint={borrarError ?? "El usuario queda inactivo y no podrá iniciar sesión. El historial se conserva. Libera un cupo del plan."}
        confirmLabel="Sí, borrar"
        cancelLabel="Volver"
        variant="danger"
        loading={borrarLoading}
        onConfirm={handleBorrar}
        onClose={() => { if (!borrarLoading) { setBorrarOpen(false); setBorrarError(null); } }}
      />
    </div>
  );
}
